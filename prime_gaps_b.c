...
int main(int argc, char** argv) {
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
		...
		unsigned long first_last_primes_ui[2]; // stores the first and last primes in this process
		...
		if (rank != 0) {
			...
			// MPI_Send(&first_last_primes_ui[1], 1, MPI_UNSIGNED_LONG, 0, 2, MPI_COMM_WORLD);
		}

		// print_mpz("gap", local_primegap[0], ", ");
		// print_mpz("prime", local_primegap[1], "\n");
		...
		if (rank == 0) {
			// mpz_t all_first_last_primes[size*2];
			// for (int i = 0; i < size*2; i++) mpz_init(all_first_last_primes[i]);

			// mpz_set(all_first_last_primes[0], first_last_primes[0]);
			// mpz_set(all_first_last_primes[1], first_last_primes[1]);
			...
			for (int i = 1; i < size; ++i) {
				...
				// MPI_Recv(&first_last_primes_ui[1], 2, MPI_UNSIGNED_LONG, i, 2, MPI_COMM_WORLD, &status);
				// mpz_set_ui(all_first_last_primes[i*2], first_last_primes_ui[0]);
				// mpz_set_ui(all_first_last_primes[i*2+1], first_last_primes[1]);
